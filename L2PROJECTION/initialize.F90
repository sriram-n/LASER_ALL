...
!
!
!  ...set GMP control parameters and AF control parameters
      select case(INPUT_FILE)
      case(DUMPIN_LEGACY_,DUMPIN_)
        NRCOMS = 1 ; MAXNRHS = 1 ; MAXEQNH = 1 ; MAXEQNE = 2
                                   MAXEQNV = 1 ; MAXEQNQ = 1
      case default
!                                     NDIM //    MANDIM //
        call set_gmp_parameters(         3 ,          3 , &
...
!  ...generate initial mesh
      ...
      case(DUMPIN_LEGACY_)
        ...
        call dumpin_hp3d_LEGACY(      'files/dumpc3Dhp')
      case(DUMPIN_)
        write(*,*)'dumping in physics...'
        call dumpin_physics_from_file('files/dumpPHYS' )
      ...
      endselect
!
!     generate HP3D Data Structure
!      call hp3gen(trim(FILE_PHYS))
!
!     set mesh refinement filters [OPTIONAL]
!!!      call set_mesh_refinement
!
!     set problem-dependent quantities [OPTIONAL]
!!      call set_my_project
!
!  ...initialize work space for the frontal solver
      call set_frsol_workspace (100000000)
!
