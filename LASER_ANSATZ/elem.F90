...
!                    +zsolQ(3)**2)*(1.d0-(1.d0/OMEGA_RATIO))*(-gainFunction))
			 zfval = (KAPPA/1.38d0)*DELTAT*((INTENSITY_RATIO**-2)*(1.d0-(1.d0/OMEGA_RATIO))*(-gainFunction))
...
! ....Maxwell load and auxiliary variables
  ...
  real*8, dimension(3) :: beta_cross_E1,beta_cross_E2,vec_cross_E1,vec_cross_E2
!
  integer,save :: ivis=0
  character    :: uplo, trans,diag
!
...
! .... for Gram matrix
  integer      :: nk
! ..... various variables for the problem
  real*8  :: h_elem,rjac,weight,wa,v2n,q,h,omeg,alpha_scale
  real*8  :: CC,CB,CE,BC,BB,BE,EC,EB,EE
...
! .....compute the H1 solution at the point
        ...
        do kH=1,nrdofH
          zsolH = zsolH + zdofH(1,kH)*shapH(kH)
        enddo
        alpha_scale = 2.d0*zsolH*ALPHA_HEAT*(REF_INDEX_CORE+REF_INDEX_CLAD)*0.5d0      
! ...  evaluate auxiliary constant1
        zb = ZI*OMEGA*(bg_gain*EPSILON + alpha_scale) - ion_gain-SIGMA
        za = ZI*OMEGA*(bg_gain*EPSILON + alpha_scale) + ion_gain+SIGMA
...
!
! ..... get ansatz Vector
	!call get_ansatz(x,AZINUM, ANSATZVEC)
! .....get the RHS
    call getf(Mdle,x, zfval,zJ)
!
...
! .......Piola transform
      ...
      call cross_product(ANSATZVEC,E1, beta_cross_E1)
      call cross_product(ANSATZVEC,E1, vec_cross_E1)
!
! .......compute the RHS
      k = 2*k1
      BLOADE(k) = BLOADE(k) &
...
! .........Piola transform
      ...
      call cross_product(ANSATZVEC,E2, beta_cross_E2)
      call cross_product(ANSATZVEC,E2, vec_cross_E2)
!
! .........auxiliary quantities
      CC  = (curlE1(1) + ZI*beta_cross_E1(1))*(curlE2(1) - ZI*beta_cross_E2(1)) &
           + (curlE1(2) + ZI*beta_cross_E1(2))*(curlE2(2) - ZI*beta_cross_E2(2)) &
           + (curlE1(3) + ZI*beta_cross_E1(3))*(curlE2(3) - ZI*beta_cross_E2(3)) 
      CE = (curlE1(1) + ZI*beta_cross_E1(1))*E2(1) + (curlE1(2) + ZI*beta_cross_E1(2))*E2(2) &
            + (curlE1(3) + ZI*beta_cross_E1(3))*E2(3)
      EC = E1(1)*(curlE2(1) - ZI*beta_cross_E2(1))+E1(2)*(curlE2(2) - ZI*beta_cross_E2(2)) &
            + E1(3)*(curlE2(3) - ZI*beta_cross_E2(3))
      EE = E1(1)*E2(1) + E1(2)*E2(2) + E1(3)*E2(3)
!      CC  = curlE1(1)*curlE2(1) + curlE1(2)*curlE2(2) &
!              + curlE1(3)*curlE2(3)
!      CB  = curlE1(1)*beta_cross_E2(1) + curlE1(2)*beta_cross_E2(2) &
!              + curlE1(3)*beta_cross_E2(3)
!      CE  = curlE1(1)*E2(1) + curlE1(2)*E2(2) + curlE1(3)*E2(3) 
!      BC  = beta_cross_E1(1)*curlE2(1) + beta_cross_E1(2)*curlE2(2) &
!              + beta_cross_E1(3)*curlE2(3)
!      BB  = beta_cross_E1(1)*beta_cross_E2(1)+beta_cross_E1(2)*beta_cross_E2(2) &
!            + beta_cross_E1(3)*beta_cross_E2(3)
!      BE  = beta_cross_E1(1)*E2(1)+beta_cross_E1(2)*E2(2)+beta_cross_E1(3)*E2(3)                    
!      EC  = E1(1)*curlE2(1) + E1(2)*curlE2(2) + E1(3)*curlE2(3)
!      EB  = E1(1)*beta_cross_E2(1)+E1(2)*beta_cross_E2(2)+E1(3)*beta_cross_E2(3)
!      EE = E1(1)*E2(1) + E1(2)*E2(2) + E1(3)*E2(3)
     
      !write(*,*) 'CB and BC are ', CB, BC
      !write(*,*) 'EB and BE are ', EB, BE
      !write(*,*) 'CE and EC are ', CE, EC
      !write(*,*) 'EB and BE are ', EB, BE
      !write(*,*) 'k2, k1 are ', k2, k1
 !     call pause
...
! .........accumulate for the extended stiffness matrix
        do icomp=1,3
          ...
          STIFFEQ(2*k1-1,k) = STIFFEQ(2*k1-1,k) &
                 + (curlE1(icomp)-ZI*vec_cross_E1(icomp))*q*weight
          k = (k2-1)*6 + 3+ icomp
          STIFFEQ(2*k1-1,k) = STIFFEQ(2*k1-1,k) &
                 +zc*E1(icomp)*q*weight
          k = (k2-1)*6 + icomp
          STIFFEQ(2*k1  ,k) = STIFFEQ(2*k1  ,k) &
               -(za)*E1(icomp)*q*weight
          k = (k2-1)*6 + 3+ icomp
          STIFFEQ(2*k1  ,k) = STIFFEQ(2*k1  ,k) &
               + (curlE1(icomp)-ZI*vec_cross_E1(icomp))*q*weight
...
!-------------------------------------------------------------------
!Routine to evaluate gain function
...
!-------------------------------------------------------------------
!Routine to evaluate ansatz vector
!Last modified : Jul 17
!input: Xi (physical coordinates)
!output: grad(ZI*arctan(y/x))+ZI*BETA
!-------------------------------------------------------------------
subroutine get_ansatz(Xp)
  use problem
  use control
  implicit none
!
  ...
  ANSATZVEC(1) = -AZINUM*(xx(2))/(xx(1)**2+xx(2)**2)
  ANSATZVEC(2) = AZINUM*(xx(1))/(xx(1)**2+xx(2)**2)
  ANSATZVEC(3) = 0.d0
  ANSATZVEC = ANSATZVEC + BETA
  
  DANSATZVEC(1,1) = 2.d0*AZINUM*xx(1)*xx(2)/(xx(1)**2+xx(2)**2)**2
  DANSATZVEC(1,2) = AZINUM*(xx(2)**2-xx(1)**2)/(xx(1)**2+xx(2)**2)**2
  DANSATZVEC(1,3) = 0.d0
  DANSATZVEC(2,1) = DANSATZVEC(1,2)
  DANSATZVEC(2,2) = -2.d0*AZINUM*xx(1)*xx(2)/(xx(1)**2+xx(2)**2)**2
  DANSATZVEC(2,3) = 0.d0
  DANSATZVEC(3,1) = 0.d0
  DANSATZVEC(3,2) = 0.d0
  DANSATZVEC(3,3) = 0.d0
end subroutine get_ansatz
