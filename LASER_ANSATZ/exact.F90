...
!
  ...
  VTYPE,dimension(3)       :: dE, beta_cross_E
  real*8, dimension(3,3)   :: grad2u
  VTYPE, dimension(3,3)    :: d2E, beta_cross_DE, Dbeta_cross_E
  real*8                   :: ut
  integer                  :: icomp

!     initialize ANSATZ FUNCTION
  call get_ansatz(Xp)
...
!     initialize variables
  ...
  beta_cross_DE=ZERO; beta_cross_E = ZERO; Dbeta_cross_E = ZERO
!
...
!  ...value
  ...
  call my_cross_product(ANSATZVEC,ValE(1:3,1), beta_cross_E)
  ValE(1:3,2) = (ValE(1:3,2)+ZI*beta_cross_E)/(-ZI*OMEGA*MU)
!
!  ...1st order derivatives
...
!
  call my_cross_product(ANSATZVEC,DvalE(1:3,1,1), beta_cross_DE(1:3,1))
  call my_cross_product(ANSATZVEC,DvalE(1:3,1,2), beta_cross_DE(1:3,2))
  call my_cross_product(ANSATZVEC,DvalE(1:3,1,3), beta_cross_DE(1:3,3)) 
  
  call my_cross_product(DANSATZVEC(1:3,1),valE(1:3,1), Dbeta_cross_E(1:3,1))
  call my_cross_product(DANSATZVEC(1:3,2),valE(1:3,1), Dbeta_cross_E(1:3,2))
  call my_cross_product(DANSATZVEC(1:3,3),valE(1:3,1), Dbeta_cross_E(1:3,3)) 
  
  
  DvalE(1:3,2,1:3) = (DvalE(1:3,2,1:3)+ZI*beta_cross_DE(1:3,1:3) + ZI*Dbeta_cross_E(1:3,1:3))/(-ZI*OMEGA*MU)
...
!
    ...
    real*8 :: cn,dn     ! for singular solution
    real*8 :: tn        ! for time step
    real*8 :: np_x,np_y,np_z,r0,k0,w0,phase,amplitude,amp
...
!
      ...
      f_x= sin(om*Xp(1))!dexp(-Xp(1)**2)/20.d0!sin(OMEGA*Xp(1))
      f_y= sin(om*Xp(2))!dexp(-Xp(2)**2)/20.d0!sin(OMEGA*Xp(2))
      f_z= sin(om*Xp(3))*cdexp(-ZI*Xp(3))
!
!     1st order derivatives
      df_x=(om)*cos(om*Xp(1))!-Xp(1)*dexp(-Xp(1)**2)/10.d0!(OMEGA)*cos(OMEGA*Xp(1))
      df_y=(om)*cos(om*Xp(2))!-Xp(2)*dexp(-Xp(2)**2)/10.d0!(OMEGA)*cos(OMEGA*Xp(2))
      df_z=(om)*cos(om*Xp(3))*cdexp(-ZI*Xp(3))-sin(om*Xp(3))*ZI*cdexp(-ZI*Xp(3))
!
!     2nd order derivatives
      ddf_x=-om**2*f_x!dexp(-Xp(1)**2)*(2.d0*Xp(1)**2-1.d0)/10.d0!-OMEGA**2*f_x
      ddf_y=-om**2*f_y!dexp(-Xp(2)**2)*(2.d0*Xp(2)**2-1.d0)/10.d0!-OMEGA**2*f_y
      ddf_z=cdexp(-ZI*Xp(3))*(-(om**2+1)*sin(om*Xp(3))-2.d0*ZI*om*cos(om*Xp(3)))
...
!.... beam waist
          w0 = BEAM_WAIST
          k0 = OMEGA
          amp = 10.d0*INTENSITY_RATIO**-2
          c2z = w0**2
          x1 = Xp(1)
          x2 = Xp(2)
          r0 = dsqrt((x1)**2+(x2)**2)
          uz = amp*cdexp(-r0**2/c2z)/c2z*cdexp(ZI*BETA(3)*Xp(3))
    !
          uz_x = -2.d0*x1*uz/c2z
          uz_y = -2.d0*x2*uz/c2z
          uz_z = uz*(ZI*BETA(3))
...
!.... plane wave parameters
          phase = BETA(3)
          amplitude = INTENSITY_RATIO**-2
          pz = amplitude*exp(ZI*Xp(3)*phase)
          pz_x = ZERO
          pz_y = ZERO
          pz_z = pz*ZI*phase
          pz_xx = ZERO
...
!     else, only plane wave in the cladding
        if((sqrt(Xp(1)**2+Xp(2)**2).le.0.5)) then
        E= uz+pz
    !  .....1st order derivatives
        dE(1) = pz_x+uz_x
        dE(2) = pz_y+uz_y
