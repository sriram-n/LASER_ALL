...
!
      ...
      integer :: iref,numRef,itag,isolver
      integer, dimension(1) :: flag,iflag
!
!----------------------------------------------------------------------
!
...
!                             option label      // explanation                // default value     // parameter
      ...
      call get_option_string( '-prefix'          , 'Prefix paraview file'      ,'hcurlProj_'            , PREFIX      )
!
!     -- Parview Interface --
...
!            call solve1(MY_NR_RHS)
            ...
            elseif (isolver.eq.2) then
              call pardiso_sc_herm
               ...
               write(*,*) 'main HCURL_PROJECTION: error: enter 1 or 2'
               goto 55
            endif
            !call unhack_solver
            !call mumps_solve_seq(MY_NR_RHS)
